...
Let a logical workflow expose candidate context blocks
\begin{equation}
X = \{x_1, x_2, \ldots, x_n\}.
\end{equation}
...
Trajectory graphs add a second source of signal. If repeated runs show the same system instructions, tool definitions, repository files, style guides, or retrieved chunks flowing into the same model-call shape, \migaki{} can report them as stable context candidates before changing execution. In v0, the safe path is:

\begin{center}
detect exact repeated context $\rightarrow$ record cache metadata $\rightarrow$ compare runs $\rightarrow$ propose reuse.
\end{center}

This is less glamorous than semantic compression, but it creates a falsifiable benchmark target. A report can say how many repeated context-bearing nodes exist, how many tokens are potentially avoidable, and which dependencies prevented reuse.
